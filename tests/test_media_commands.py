...
def test_flacted_main_show_tag(
    ...
    mocker.patch(
        'deltona.commands.media.sp.run', return_value=mocker.MagicMock(stdout='TITLE=Test\n')
    )
...
def test_flacted_main_show_tag_year(
    ...
    mocker.patch(
        'deltona.commands.media.sp.run', return_value=mocker.MagicMock(stdout='DATE=Test\n')
    )
    result = runner.invoke(flacted_main, [str(f)])
    assert result.exit_code == 0
