...
def test_fix_chromium_pwa_icon_main(
    ...
    assert result.exit_code == 0
    mock_fix_icon.assert_called_once_with(
        mocker.ANY, 'app_id', 'src_uri', 'Default', masked=False, monochrome=False
    )


def test_check_bookmarks_html_main(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
...
def test_chrome_bisect_flags_main_no_flags(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
...
def test_chrome_bisect_flags_main_3_flags(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
    mocker.patch('deltona.commands.www.kill_processes_by_name')
    mock_local_state = tmp_path / 'Local State'
    mock_local_state.write_text(
        json.dumps({'browser': {'enabled_labs_experiments': ['a', 'b', 'c']}})
    )
...
def test_chrome_bisect_flags_main_4_flags(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
    mocker.patch('deltona.commands.www.kill_processes_by_name')
    mock_local_state = tmp_path / 'Local State'
    mock_local_state.write_text(
        json.dumps({'browser': {'enabled_labs_experiments': ['a', 'b', 'c', 'd']}})
    )
    result = runner.invoke(
        chrome_bisect_flags_main, [str(mock_local_state)], input=('\n\nn\n\ny\n\nn\n\ny\n')
    )
    assert result.exit_code == 0
    assert 'Saved "Local State" with "d" removed.' in result.output


def test_chrome_bisect_flags_main_invalid_response(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
    mocker.patch('deltona.commands.www.kill_processes_by_name')
    mock_local_state = tmp_path / 'Local State'
...
def test_chrome_bisect_flags_main_single_flag(
    mocker: MockerFixture, runner: CliRunner, tmp_path: Path
) -> None:
    mocker.patch('deltona.commands.www.kill_processes_by_name')
    mock_local_state = tmp_path / 'Local State'
