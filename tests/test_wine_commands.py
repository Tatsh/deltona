...
def test_mkwineprefix_main_success(
    mocker: MockerFixture, runner: CliRunner, fake_prefix: Path
) -> None:
    create_wine_prefix = mocker.patch(
        'deltona.commands.wine.create_wine_prefix', return_value=str(fake_prefix)
    )
...
def test_mkwineprefix_main_subprocess_error(mocker: MockerFixture, runner: CliRunner) -> None:
    ...
    mocker.patch(
        'deltona.commands.wine.create_wine_prefix',
