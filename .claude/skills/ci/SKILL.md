...
If any changed files are under `deltona/` or `tests/`, run the following
...
1. **click-auditor** - validate Click command consistency. **Only run if the project directly uses
   Click and files under `deltona/commands/` changed.**
...
- **changelog** - update `CHANGELOG.md` only for **user-facing** changes (behaviour, CLI, public
  ...
  Files under `deltona/`, `tests/`, or version changes in `pyproject.toml` are **candidates**
